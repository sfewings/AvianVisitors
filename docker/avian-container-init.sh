...
# An ALSA capture device admits exactly one reader. birdnet_recording holds it
# with arecord, so livestream's ffmpeg gets "Device or resource busy" and the
# live audio never starts. Both read REC_CARD, so both open the same PCM.
#
# Upstream sidesteps this by recommending REC_CARD=default, which on a Pi routes
# through PulseAudio and mixes several readers. This container does not run a
# sound server, so "default" would otherwise be meaningless here.
#
# dsnoop is ALSA's own answer: a plugin that fans one capture stream out to
# several clients, no daemon involved. Defining it as pcm.!default restores the
# behaviour upstream expects, and REC_CARD=default then works as documented.
#
# Opt-in via ALSA_HW_DEVICE, because it only matters if you want the livestream:
# recording alone is happier talking straight to the hardware.
setup_alsa_sharing() {
  local hw="${ALSA_HW_DEVICE:-}"
  [ -n "$hw" ] || return 0

  # dsnoop's slave must be raw hardware. plughw is itself a conversion plugin,
  # and layering it underneath dsnoop either fails outright or silently defeats
  # the fixed-format contract dsnoop needs. Users will naturally paste their
  # REC_CARD value here, which is usually the plughw form, so normalise it.
  case "$hw" in
    plughw:*)
      hw="hw:${hw#plughw:}"
      log "ALSA: using $hw as the dsnoop slave (plughw cannot be one)"
      ;;
  esac

  # dsnoop cannot resample or re-channel: its slave parameters must match what
  # the hardware actually does, unlike plughw which converts silently. Most USB
  # lavalier mics are mono, hence the default; override if arecord reports
  # Stereo for your device.
  local ch="${ALSA_HW_CHANNELS:-1}"
  local rate="${ALSA_HW_RATE:-48000}"

  log "ALSA: sharing $hw via dsnoop (channels=$ch rate=$rate)"
  cat > /etc/asound.conf <<EOF
# Generated by avian-container-init. Edits are overwritten on restart; change
# ALSA_HW_DEVICE / ALSA_HW_CHANNELS / ALSA_HW_RATE instead.
#
# dsnoop fans one capture stream out to several readers, so birdnet_recording
# and livestream can both have the mic. Without it the second one to start gets
...
# plug wraps it so callers asking for a different channel count or sample rate
# are converted rather than refused.
pcm.!default {
    type plug
    slave.pcm "avian_dsnoop"
}
EOF
...
check_audio() {
  ...
  case "$rc" in
    default)
      if [ -n "${ALSA_HW_DEVICE:-}" ] && grep -q avian_dsnoop /etc/asound.conf 2>/dev/null; then
        log "audio source: ALSA 'default' -> dsnoop over ${ALSA_HW_DEVICE}"
        log "  shared, so birdnet_recording and livestream can both capture."
        return 0
      fi
      log "WARNING: REC_CARD is 'default' but nothing provides a default device."
      log "         This container runs no sound server, so 'default' resolves"
      log "         to nothing. Either set REC_CARD to one of the devices above,"
      log "         or set ALSA_HW_DEVICE to share one via dsnoop (needed if you"
      log "         want the livestream as well as recording)."
      return 0
      ;;
    *CARD=*)
  ...
  if [ -e "/dev/snd/pcmC${want}D0c" ]; then
    ...
    # Direct hardware access is exclusive: whichever of birdnet_recording and
    # livestream starts first keeps the mic, and the other logs
    # "Device or resource busy" forever. Only worth mentioning when the
    # livestream is actually meant to be running.
    if [ -z "${ALSA_HW_DEVICE:-}" ] && \
       [ "$(systemctl is-enabled livestream 2>/dev/null)" = enabled ]; then
      log "  NOTE: this is exclusive access, so livestream cannot also open the"
      log "        mic and will log 'Device or resource busy'. To run both:"
      log "          ALSA_HW_DEVICE=hw:${rc#plughw:}"
      log "          REC_CARD=default"
      log "        which shares the device through ALSA's dsnoop plugin."
    fi
  else
